...
export default function GenerateFeedbackForm() {
    ...
    const handleSaveSubmit = async (e) => {
        ...
        if(FormId){
            try {
                const response = await fetch(`/api/updateFeedbackFormFields/${FormId}`, {
                  method: 'PUT',
                  headers: {
            ...
            try {
                const response = await fetch(`/api/generateFeedbackform`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
    ...
    useEffect(() => {
        if (FormId) {
          fetch(`/api/updateFeedbackForm/${FormId}`)
    ...
    const handleDelete = async (FormChildId) => {
      try {
        const response = await fetch(`/api/DeleteFormChild/${FormChildId}`, {
          method: 'DELETE',
        });
